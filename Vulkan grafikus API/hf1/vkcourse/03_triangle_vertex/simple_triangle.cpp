                                ...
                                const VkShaderModule   shaderFragment)
{
    ...
    const VkVertexInputBindingDescription bindingInfo = {
        // TASK:
        // 2) Specifly the buffer's layout:
        //    * What is the index of buffer during buffer bind (see draw commands)?
        //    * What is the "step" beteween vertex elements (in bytes)?
        //    * When to switch to next vertex element?
        .binding   = 0,
    ...
    const VkVertexInputAttributeDescription attributeInfo = {
        // TASK:
        // 3) Specifly the vertex attributes info:
        //    * Which location is this connected to in the shader?
        //    * Which buffer binding is it connected to (see previous VkVertexInputBindingDescription struct)?
        //    * What is the format of the input date (per vertex)?
...
VkResult SimpleTriangle::Create(const Context& context, const VkFormat colorFormat)
{
    ...
    std::vector<Vertex> vertexData = {
        // TASK:VkBufferUsageFlags usageFlagsVkBufferUsageFlags usageFlags
        // 1) Add triangle coordinates in Counter Clokwise order (CCW)
        {-0.5f, -0.5f, 0.0f},
        {-0.5f, 0.5f, 0.0f},
        {0.5f, -0.5f, 0.0f},
    };
...
void SimpleTriangle::Draw(const VkCommandBuffer cmdBuffer)
{
    ...
    mvp           = glm::rotate(mvp, glm::radians((float)tick++), glm::vec3(0.0f, 0.0f, 1.0f));
    // MVP[0][0] = ...;
    // MVP[0][1] = ...;
    // MVP[1][0] = ...;
    // MVP[1][1] = ...;
